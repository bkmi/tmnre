...
The data in this repository is available using [Git LFS](https://git-lfs.github.com/). By default, the necessary data is replaced by small text files to keep downloading easy. If you're interested in the data, download and install Git LFS. Once you've cloned this repository,  get the large files (>15GB) using the following command.

```setup
git lfs pull --include="*" --exclude=""
```

The large files include marginalized C2ST calculations from `sbibm`, direct marginal inference results using `tmnre`, and simulation data for the physics example.
...
We have provided a results file already calculated in `marginalize_sbibm/marginal-c2st-summary.csv`. It was computed using the raw data from our marginalization runs. Those files are stored in `marginalize_sbibm/raw/` using Git LFS.
...
We have provided a file already computed in `inference_sbibm/reports/swyft_uniform_2d_results_budget.csv`. It was computed using the raw data from our experimental runs. Those files are stored in `inference_sbibm/raw/` using Git LFS.
